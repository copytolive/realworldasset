  ...
  return <main className={`rwa-onboarding-page ${softTheme?"soft-theme":""}`}>
    ...
    <section className="rwa-onboarding-shell">
      ...
      <div className="rwa-onboarding-main">
        ...
        <footer className="rwa-onboarding-footer">{step<4&&<><Button variant="secondary" onClick={()=>step===1?finish():setStep(s=>s-1)}>{step===1?"Skip for now":"Back"}</Button><Button onClick={()=>{if(step===3&&name.trim().length<2)setName("RWA Explorer");goNext();}}>{step===1?"Continue →":step===2?"Continue →":"Save & Continue →"}</Button></>}</footer>
      </div>
    </section>
