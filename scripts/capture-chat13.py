...
VIEWPORT={"width":1672,"height":941};ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"visual-audit";PREVIEW=ROOT/".visual-preview-chat13";SITE=PREVIEW/"realworldasset"
SURFACES={"69-business-orders":"/merchant/orders/","52-update-composer":"/merchant/updates/new/","53-advertising-manager":"/merchant/ads/"}
def prepare():
...
def audit(page,base,name):
    ...
    if fail:raise RuntimeError("CHAT13 button audit failed: "+"; ".join(f'{x["surface"]} #{x["index"]} {x["label"]}: {x.get("error") or "no observable action"}' for x in fail[:30]))
    print(f"CHAT13 button audit PASS: {name} — {len(rows)} visible buttons checked");return rows
...
def main():
    ...
    (OUT/"chat13-capture.json").write_text(json.dumps(records,indent=2));(OUT/"chat13-button-audit.json").write_text(json.dumps(audits,indent=2));print(f"Captured {len(records)} CHAT13 surfaces at 1672x941; {sum(x['status'].startswith('PASS') for x in audits)} controls passed/current-state")
