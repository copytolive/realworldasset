...
def reset(page,base,name):
    goto(page,base,SURFACES[name]); instrument(page)
...
def audit_surface(page,base,name):
    ...
    for i in range(count):
      ...
      if m.get("ariaPressed")=="true" or m.get("ariaSelected")=="true" or m.get("dataActive")=="true" or label in CURRENT_STATE_LABELS.get(name,set()):
        rows.append({"surface":name,"index":i,"label":label,"status":"PASS_CURRENT_STATE"}); continue
      before=state(page); err=None
      try:
        b.click(timeout=3000); page.wait_for_timeout(220)
    ...
    if fail:
      summary="; ".join(f'{x["surface"]} #{x["index"]} {x.get("label")}: {x.get("error") or "no observable action"}' for x in fail[:30])
      raise RuntimeError(f"CHAT09 browser button audit failed ({len(fail)}): {summary}")
    print(f"CHAT09 button audit PASS: {name} — {len(rows)} visible buttons checked")
    return rows

def verify_flow(page,base):
    goto(page,base,"/businesses/kopi-nusantara/")
    page.get_by_role("button",name="Visit Store",exact=False).first.click(timeout=4000); page.wait_for_timeout(250)
...
def main():
    prepare(); server=subprocess.Popen(["python3","-m","http.server","4179","--bind","127.0.0.1","--directory",str(PREVIEW)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(1)
    base="http://127.0.0.1:4179/realworldasset"; records=[]; audits=[]
    try:
    ...
    (OUT/"chat09-capture.json").write_text(json.dumps(records,indent=2),encoding="utf-8")
    (OUT/"chat09-button-audit.json").write_text(json.dumps(audits,indent=2),encoding="utf-8")
    passed=sum(1 for x in audits if x["status"].startswith("PASS")); skipped=sum(1 for x in audits if x["status"].startswith("SKIP"))
    print("Captured 5 CHAT09 source-of-truth surfaces at 1672x941")
    print(f"CHAT09 browser button audit PASS: {passed} functional/current-state controls; {skipped} intentional skips")
