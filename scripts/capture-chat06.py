...
PREVIEW=ROOT/".visual-preview-chat06"
SITE=PREVIEW/"realworldasset"
SURFACES=[
 ("19-notifications-center","/notifications/",None),
...
def reset(page,base,route,opener):
    goto(page,base,route); open_state(page,opener); instrument(page)

def audit_surface(page,base,name,route,opener):
    reset(page,base,route,opener)
    ...
    for i in range(count):
      ...
      try:
        b.click(timeout=2500); page.wait_for_timeout(130)
    ...
    if fail:
      ...
      raise RuntimeError(f"CHAT06 browser button audit failed ({len(fail)}): {summary}")
    print(f"CHAT06 button audit PASS: {name} — {len(out)} visible buttons checked")
    return out

def main():
    prepare(); server=subprocess.Popen(["python3","-m","http.server","4176","--bind","127.0.0.1","--directory",str(PREVIEW)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); time.sleep(1)
    base="http://127.0.0.1:4176/realworldasset"; records=[]; audits=[]
    try:
      with sync_playwright() as p:
        ...
        for name,route,opener in SURFACES: records.append(capture(page,base,name,route,opener))
        for name,route,opener in SURFACES: audits.extend(audit_surface(page,base,name,route,opener))
        browser.close()
    finally:
      server.terminate()
      try: server.wait(timeout=5)
      except subprocess.TimeoutExpired: server.kill()
    (OUT/"chat06-capture.json").write_text(json.dumps(records,indent=2),encoding="utf-8")
    (OUT/"chat06-button-audit.json").write_text(json.dumps(audits,indent=2),encoding="utf-8")
    passed=sum(1 for x in audits if x["status"].startswith("PASS")); skipped=sum(1 for x in audits if x["status"].startswith("SKIP"))
    print("Captured 5 CHAT06 source-of-truth surfaces at 1672x941")
    print(f"CHAT06 browser button audit PASS: {passed} functional/current-state controls; {skipped} intentional skips")
