...
def save_crop(image: Image.Image, box, name: str, *, jpeg=False):
    crop = image.crop(box).convert("RGB")
    path = ASSETS / name
    if jpeg:
        crop.save(path, "JPEG", quality=84, optimize=True)
    else:
...
# Brand and featured-business logos from 01.
...
# Pure photo regions from the interest cards in 03. UI text is deliberately excluded.
interest = {
...
for name, box in interest.items():
    save_crop(refs["03"], box, name, jpeg=True)

# Follow avatars from 03.
...
# Wallet icons from 05; reused by login, wallet modal and manager.
